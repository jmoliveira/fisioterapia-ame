...
function sendProfileToServer(){
	$.ajax({
			type:'POST',
			data:{log_profile: JSON.stringify(jQuery.getProfile())},
			url:'/admin/estrutura/debug/profile/log',
			success:function(json) {
				jQuery.clearProfile()
			},
			error:function(){
				$.log('Erro ao tentar enviar o log de profile ao servidor.');
			}
		});
	}

setInterval( 'sendProfileToServer()', 30000);
$(window).unload( function () {sendProfileToServer();} );
