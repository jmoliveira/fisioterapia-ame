...
*
*  Last Review: 18:00 PM 3/25/2009
*
*/
...
(function($) {
  ...
  var fnFormatResult = function(value, data, currentValue) {
    var pattern = currentValue.replace(/([.*+?|(){}[\]\\])/g, '\\$1');
    return value.replace(new RegExp(pattern, 'gi'), '<strong>' + currentValue + '<\/strong>');
  };
  ...
  Autocomplete.prototype = {
    ...
    suggest: function() {
      ...
      for (var i = 0; i < len; i++) {
        div = $((me.selectedIndex === i ? '<div class="selected"' : '<div') + ' title="' + this.suggestions[i] + '">' + f(this.suggestions[i], this.data[i], this.currentValue) + '</div>');
